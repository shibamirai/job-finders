...
<div class="max-w-4xl mx-auto bg-white shadow-md rounded px-10 py-8 mb-4">
    ...
    @if ($work)
    <form action="{{ route('works.update', ['jobFinder' => $jobFinder, 'work' => $work]) }}" method="POST">
        @method('PATCH')
    @else
    <form action="{{ route('works.store', $jobFinder) }}" method="POST">
    @endif
    ...
    <form id="delete-{{ $work->id }}" method="POST"
        action="{{ route('works.delete', ['jobFinder' => $jobFinder, 'work' => $work]) }}"
    >
        @method('DELETE')
        @csrf
    </form>
