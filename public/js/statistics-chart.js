...
finders.forEach(function (finder) {
  ...
  if (/エンジニア/.test(finder.occupation) || /プログラマ/.test(finder.occupation) || /コーダ/.test(finder.occupation) || /デザイナ/.test(finder.occupation)) {
    genres.data[0]++;
  } else {
    genres.data[1]++;
  }
...
employment_pattern.labels.push(employment_pattern.labels.shift()); // ツールチップを常に表示するための処理
